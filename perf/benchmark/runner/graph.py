...
def plotter(args):
    ...
    for telemetry_mode in args.telemetry_modes:
        ...
        print("----")
        print(telemetry_modes_y_data[telemetry_mode])
...
def get_data_helper(df, query_list, query_str, telemetry_mode, metric_name):
    ...
    print(query_list)
    print(query_str)
    print(telemetry_mode) 
    print(metric_name)
    query_str = "NumThreads==@ql and Labels.str.endswith(@telemetry_mode)"
...
def get_parser():
    parser = argparse.ArgumentParser(
        "Flomesh performance benchmark CSV file graph plotter.")
    parser.add_argument(
        "--graph_type",
